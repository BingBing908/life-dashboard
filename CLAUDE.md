...
- **学习记录 `study-log`**（**2026-07-20 改版为六大板块 v2**，原「科目+时长」已弃）：**主界面＝六个彩色方框**（田字：英语·语文 / AI·历史 / 书籍·电影），每框左图标+名+条数、**右侧露出内容预览**（学习板块显示最新一条摘要；书籍/电影露封面缩略）；点方框进对应板块、左上"← 学习记录"返回（模块内自管 `board` state，非路由）。数据＝**一张通用表 `study_entries`**（迁移 **v12**：board/kind/entry_date/title/body/meta(JSON)/status + 同步字段；`data.ts` 里 `listAllEntries`(一次取全在组件里分组)/`createEntry/updateEntry/deleteEntry`）。
  - **每条内容「今日看完」判定**（2026-07-21，统一函数 `entryDone(e)`）：**要默写的内容（古诗/精读文章 `needsDictation`）＝默写过≥1遍才算看完**（读 meta.artAtt/wordAtt 长度，头部只读徽标「默写后算看完/✓已默写」，不能手动标）；**练笔＝交了作业即完成**（2026-07-27，`entryDone` 里 kind==='练笔' 时看 `meta.homework` 有没有——Claude 批改后写进 homework 就自动算完成，头部只读徽标「交作业后算完成/✓已完成」，不手动标）；**其余内容＝手动标 `meta.done`**（头部「标看完/✓已看完」开关）。看完的条目变淡。没标/没默＝没来得及、内容不换明天接着看。Landing 每学习板块显示「今日 X/Y 看完」（今天条目里 entryDone 的比例）。
