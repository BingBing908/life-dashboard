...
注册在 `registry.ts` 的有 6 个：`focus`（此刻，首位）、`todo`（待办）、`study-plan`（学练计划）、`supplement`（饮食）、`study-log`（学习记录）、`mini-table`（小表格）。

- **此刻 `focus`**（日常主入口，B2 左时间轴布局）：左侧连线时间轴列六个领域（养生/英语/工作/学习/运动/阅读，各带 start 分钟数+颜色），按当前时间自动定位到对应领域（`autoDomainKey`：最后一个 start≤now），红色虚线标"现在"；点领域可手动切、"回到此刻"复位。右侧是该领域的**三行小框**：①事项+打开按钮 ②网址（可点）③"我做了什么"进度笔记。笔记门控：英语/学习/阅读/工作**必填才能勾**，养生/运动选填。数据源：养生/英语/学习/运动/阅读=study-plan 对应 track；**工作=todo 里 due_date≤今天的项**（复用 plan_notes 存笔记、toggleTodo 打勾）。复用 study-plan 的 applyPeriod（经期），plan 条目走 plan_checks/plan_notes。
