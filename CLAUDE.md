...
3. **英语**（工作时间抽 ≤1 小时；与晨间「新概念」块是两段英语，合计偏重，故**已定稿精简**）：
   - 每日一篇**精读文章**（Claude 按当下水平≈A2 挑选、要短），支持**划生词**（存所划的词）；给文章时同时给：精读教程/引导 + 本文值得学习的点 + 值得背诵的句子。
...
- **落地**：在 `src/lib/db.ts` 的运行环境判断里加一层 Supabase 数据源；建与现有表对应的 Postgres 表；写一次性「本地→云」迁移把她现有数据推上去（否则公司端是空的）。
- **隐私档位（待 Rosie 拍板，二选一）**：
  - **A 无登录+公开**：anon key + 宽松 RLS，打开网页即见数据、零摩擦；缺点是知道网址的人可读/写（她说不介意数据公开，但公开**写**有被乱改风险，个人小工具风险低）。
  - **B 邮箱登录**：Supabase Auth 魔法链接 + 每用户 RLS，完全私有；缺点是每设备/每会话要登录一次。
