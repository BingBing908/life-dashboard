...
- 通用组件/工具：`src/components/EditableText.tsx`（点文字就地编辑）；`src/components/DoneToggle.tsx`（计划项完成状态**两按钮【已完成】【未完成】**，替代方框勾，切换语义+可选 canComplete 门控，学练计划今日卡片/一周列表/待办页镜像三处共用）；`src/lib/openLink.ts`（Tauri 系统浏览器 / 网页新标签，全应用共用，别再各写一份）；日期用 `src/lib/dates.ts`、周几用 study-plan/data 的 `dayNumOf`（唯一实现，别在模块里重写）。
