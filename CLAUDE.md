...
- **学习记录 `study-log`**（**2026-07-20 改版为六大板块**，原「科目+时长」已弃）：Tab 顺序＝英语/语文/AI/历史/书籍/电影（Rosie 指定的田字排：英语·语文 / AI·历史 / 书籍·电影）。**一张通用表 `study_entries`**（迁移 **v12**：board/kind/entry_date/title/body/meta(JSON)/status + 同步字段）承载全部板块，`data.ts` 里 `listEntries/createEntry/updateEntry/deleteEntry/countByBoard`。**英语/语文/AI/历史**＝通用条目（带 kind 子类型下拉：精读文章·背诵·谚语 / 成语·古诗·练笔 / 新闻·术语卡 / 时间线·事件人物），加条目=标题+内容 textarea；**内容由 Claude 在对话里生成、Rosie 贴进 body 存档**（App 不内嵌模型，她只在对话里"喊更新"）。**书籍**＝在读/读完切换（status，读完把 finish_date 存进 meta JSON）+ 读后感(body)；**电影**＝片名+观影日(entry_date)+观后感。⚠️ **含输入框的卡片组件（AddEntryForm/EntryCard/BookCard/MovieCard/AddTitleForm）全部定义在模块顶层**，body/title 用**非受控 defaultValue + onBlur 存**（避免嵌套重渲染失焦）。生词划线、时间线可视化等留 v2。
