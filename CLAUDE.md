...
3. **英语**（她计划每天工作时间抽 ≤1 小时完成；注意这与晨间「新概念」块是**两段**英语，合计负荷偏重——见路线图下我的建议，倾向精简/轮换）：
   - 每日一篇**精读文章**（Claude 按她当下水平≈A2 挑选、要短），支持**划生词**（存所划的词）；给文章时同时给：精读教程/引导 + 本文值得学习的点 + 值得背诵的句子。
   - 每日一篇**背诵短文**（尽量短小简单，她的水平约半小时能背完），次日早晨复习时间背。
   - 每日一句**英语谚语/歇后语**。
4. **语文**：成语 + 古诗（建议再加：练笔/随笔输出——输出最提升；成语配典故；易错字词）。
5. **历史**（内容待定——建议：先搭朝代时间线/框架，再每日一事件或一人物卡片、须靠谱来源，配一本可靠通史；延续 Rosie「史观框架未稳前不碰偏颇通俗史」的原则）。
6. **AI**：每日 5 篇 Claude 精选高质量 AI 新闻（建议每条附一句「对 AI PM 的意义」+ 每日一个 AI 术语卡，呼应她转型 AI 产品经理的目标）。
...
- [ ] **学习记录改版：六大板块**（书籍/电影/英语/语文/历史/AI），详见「学习记录模块改版规划」（2026-07-20 提出）
