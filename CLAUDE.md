...
- **小表格 `mini-table`**：自定义列的轻量表格，是"随手建表"的基础。**已建两张常用表**（2026-07-21，直接写进云 mini_tables/mini_table_rows，非硬编码）：**三餐表格**（列＝项目+周一~周日；行＝空腹体重/早餐/午餐/晚餐/总摄入卡路里/睡前体重）、**每周运动表格**（同列；行＝养生/健身，填每天做了什么）。**动态卡路里工作流**（Rosie 认可）：她每晚称体重填进三餐表格，Claude 按**早晚体重+趋势**在**安全区（约 1420–1550、不低于 BMR 1420）**给"次日该吃多少卡（动态非定值）+ 饮食建议"，写进 `app_settings.cal_target`；运动消耗/摄入卡路里由 Claude 在对话里估算、Rosie 填表。⚠️ 支持性建议、非医疗处方（同金融红线）。
