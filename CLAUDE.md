...
- [ ] **时间轴改版（已定方案，待做）**：tab 变 **当前 / 今天 / 一周 / 路线**。「当前」＝竖线时间轴自动跟随时间（左栏用**方案 A**：每个领域节点挂迷你进度条 + 顶部总进度）+ 右栏用**方案 C**（按状态分栏，**三态分清**：待做 / 已完成 / 今天做不了）；「今天」＝**紧凑清单**（一条一行、13 条一屏、不放笔记框和详解）。设计稿见会话里的两个 artifact
