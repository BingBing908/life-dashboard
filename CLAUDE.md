...
- **补剂 `supplement`**：纯静态展示模块（无 DB），按 Rosie 作息表把每天的补剂分早/午/晚硬编码在 `SCHEDULE`（1=周一..7=周日）。Card 显示今日三段，Page 显示整周表格、今天行高亮。改补剂就改 `SCHEDULE` 常量。**接了 study-plan 的经期开关**：`getPeriodOn()` 为真时显示"经期停用"（Rosie 经期不吃补剂）——即那个🩸开关同时管运动避腹 + 补剂停用。
