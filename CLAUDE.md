...
- **学练计划 `study-plan`**：五线周计划（运动/英语/HCIP/AI方向/阅读），按 Rosie 作息表带时间段。表 `plan_items`（days: `'*'`=每天或 `'1,3,5'`，1=周一）+ `plan_checks`（每日打卡，UNIQUE(item_id,date)），迁移 v5。今日/一周两个视图，今日可勾选，条目带跟练视频链接（Tauri 用 plugin-opener 开系统浏览器，网页开新标签）。4 周递进周期（适应/完整/加量/减载），起点存 `app_settings.plan_cycle_start`。首次打开自动播种 `seed.ts`（内容按 Rosie 的运动康复需求+三线学习计划定制，注意 seedIfEmpty 有 StrictMode 并发守卫）。阅读书单原则：历史类只收史实可靠的（学术著作或明确标注虚构的史实扩写），回避被史学界指出个人偏颇的通俗史（如《明朝那些事儿》——Rosie 要求等自己历史体系完善后再读）。
