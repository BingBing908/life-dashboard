...
- **小表格 `mini-table`**：自定义列的轻量表格；**Rosie 把它当周/月复盘中枢**。已建两张常用表（2026-07-21，直接写进云 mini_tables/rows）：**三餐表格**（`tbl-meals-week`，项目+周一~日；行 空腹体重/早餐/午餐/晚餐/总摄入卡路里/睡前体重）、**运动表格**（`tbl-exercise-week`，2026-07-20 由「每周运动表格」改名；行 养生/健身）。**数据源绑定（2026-07-20）**：`sources.ts` 的 `TABLE_SOURCES` 按**表 id**把某些行接到其它模块自动填（不靠行名、不复制数据、每次打开实时算、不改库/不用 SQL）：声明 `itemCol`(标签列)+`dayCols`(mon~sun)+`autoItems`(哪些行自动)+`compute(周日期)→{项目:{列id:值}}`；TableDetail 里自动格只读浅色、其余格手填存 `mini_table_rows`。已绑定：三餐表格（早/午/晚←meal_log 内容、总摄入卡路里←dayCalories；空腹/睡前体重手填）、运动表格（养生/健身=当天学练计划里点了已完成的 wellness/sport 条目）。当前填**本周**（月度复盘待定）；将来「学习表格」加一条绑定即可。**动态卡路里工作流**（Rosie 认可）：她每晚称体重填进三餐表格，Claude 按**早晚体重+趋势**在安全区（约 1420–1550、不低于 BMR 1420）给次日 `app_settings.cal_target` + 饮食建议；运动消耗/摄入由 Claude 估算、Rosie 填表。⚠️ 支持性建议、非医疗处方（同金融红线）。
