...
/**
 * ⚠️ AI 主线的口径**以「路线」页为准**（`roadmap.ts`，2026-09-22 定稿的七阶段）——
 * 这里只写 12/27 验收当天该处在哪一步，别再在这儿维护第二份项目清单（2026-09-22 她点名
 * 「改成一致，跟现在对齐」，原来那句还写着 9/1 的旧口径「2 个作品：NOC Sentinel + life-dashboard」）。
 */
export const SEMESTER_TARGET =
  "2026-12-27 验收总目标：体重 ≤58kg 达标 / ≤56.5kg 优秀（55kg 顺延至 2027 Q1，50kg 不设为目标）· 英语学完新概念一册、开二册，无字幕看《查莉》听懂 50-60%（A2 起步，务实目标）· 【主线】按「路线」页七阶段走，12/27 该在阶段三（工程落地）末尾：三个项目已定型——Abuse 分类 Agent（真做过·主打）+ 售后知识库 Agent、工单归类 Agent（两个都标「方案设计·未落地」），且能答「怎么降 Token 成本」「RAG 答不准怎么排查」；作品集输出在 1 月（阶段四），简历重写+模拟面试在 2 月（阶段五），3–4 月投递 · ⏸ 华为认证已于 2026-09-01 暂停（Rosie：「浪费时间」），原周三晚 + 周六下午两个时段全部并给 AI；网络背景仍可作简历差异化，想恢复时把 seed.ts 里那两条加回来即可";
...
export const LINE_TARGETS: {
  ...
  { key: "ai", name: "AI", target: "三个项目定型（1 真做 + 2 设计）" },
